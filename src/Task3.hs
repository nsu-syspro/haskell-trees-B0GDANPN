...
mlookup k tree = case tlookup cmpKeys (k, undefined) tree of
    Just (_, v) -> Just v
    Nothing     -> Nothing
...
minsert k v = tinsert cmpKeys (k, v)
...
mdelete key = tdelete cmpKeys (key, undefined)

-- utils
cmpKeys :: Ord k => Cmp (k, v)
cmpKeys (keyLeft, _) (keyRight, _) = compare keyLeft keyRight